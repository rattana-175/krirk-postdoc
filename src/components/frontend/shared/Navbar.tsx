...
export default function Navbar() {
    ...
    const [isStaff, setIsStaff] = useState(false) // สร้าง state สำหรับเก็บค่า is_staff

    useEffect(() => {
        // เปลี่ยนจาก localStorage เป็น cookies
        const token = Cookies.get("accessToken") // ดึงค่า token จาก cookie
        const userStr = Cookies.get('user') // ดึงข้อมูลผู้ใช้จาก cookie
        const userData = userStr ? JSON.parse(userStr) : null
        setIsLoggedIn(!!token)
        setUser(userData)
        
        // ตรวจสอบว่า user มีค่า is_staff เป็น true หรือไม่
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center">
                ...
                <div className='hidden md:flex items-center space-x-8'>
                    {isStaff ? (
                        <Link href="/admin-postdoc" className="text-gray-600 hover:text-green-500">
                            Search
                        </Link>
                    ) : (
                        <Link href="/find-postdoc" className="text-gray-600 hover:text-green-500">
                            Search
                        </Link>
                    )}
